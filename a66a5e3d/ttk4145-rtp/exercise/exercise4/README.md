...
- Why would we aim to detect success in results, instead of the more classical way of detecting errors/failures?
 - acceptance test strategy; failure is not an option, it is a given, enumerating all errors is impossible
 - checking for success and in all other cases restart to a save checkpoint is more repellant against all (also unknown) bugs
...
- Why would we want to self-terminate, instead of handling the error immediately in the "primary" (as opposed to deferring it to the backup as it "spins up")?
 - when we self-terminate we are sure that a backup will take over the left tasks
 - because we are lazy, merging all error modes is less complex; if we have a backuup available - use is!
 - enumerating all errors is impossible
...
- Is there any reason to prefer a process pair style, as opposed to making a separate supervisor-like program whose sole purpose is to restart the main program?
 - who restarts the supervisor program when it crashes?
 - need some kind of backup for the good data either way
